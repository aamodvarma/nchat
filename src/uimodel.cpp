...
void UiModel::Impl::Init() { m_View->Init(); }
...
void UiModel::Impl::SendMessage() {
  ...
  sendMessageRequest->chatMessage.mentions =
      ParseMentions(profileId, chatId, sendMessageRequest->chatMessage.text);
...
void UiModel::Impl::EntryKeyHandler(wint_t p_Key) {
  ...
  static wint_t keyDeleteLineAfterCursor =
      UiKeyConfig::GetKey("delete_line_after_cursor");
  static wint_t keyDeleteLineBeforeCursor =
...
void UiModel::Impl::SetTyping(const std::string &p_ProfileId,
                              const std::string &p_ChatId, bool p_IsTyping) {
  static const bool typingStatusShare =
      UiConfig::GetBool("typing_status_share");
  if (!typingStatusShare)
    return;
  ...
  if (((nowTime - lastTypeTime) > 3000) && !p_IsTyping && lastIsTyping) {
    ...
    std::shared_ptr<SendTypingRequest> sendTypingRequest =
        std::make_shared<SendTypingRequest>();
  ...
  if (p_IsTyping) {
    if ((p_ProfileId == lastProfileId) && (p_ChatId == lastChatId) &&
        (p_IsTyping == lastIsTyping)) {
      if (HasProtocolFeature(p_ProfileId, FeatureTypingTimeout) &&
          ((nowTime - lastSendTime) > 2500)) {
...
void UiModel::Impl::OnKeyUnreadChat() {
  ...
  if (!unreadVec.empty()) {
    if (!unreadIsSelected) {
      ...
      auto it =
          std::find(unreadVec.begin(), unreadVec.end(), m_CurrentChatIndex);
      if (it != unreadVec.end()) {
...
void UiModel::Impl::OnKeyPrevPage() {
  ...
  int addOffset =
      std::min(historyShowCount, std::max(messageCount - messageOffset - 1, 0));
  LOG_TRACE("count %d offset %d addoffset %d", messageCount, messageOffset,
            addOffset);
  if (addOffset > 0) {
...
bool UiModel::Impl::OnKeyHomeFetchCache() {
  ...
  if (!fetchedAllCache) {
    ...
    std::shared_ptr<GetMessagesRequest> getMessagesRequest =
        std::make_shared<GetMessagesRequest>();
...
void UiModel::Impl::OnKeyHomeFetchNext() {
  ...
  if (addOffset > 0) {
    for (int i = 0; i < addOffset; ++i) {
      messageOffsetStack.push(
          1); // @todo: consider building a nicer stack for page down from home
    }
...
void UiModel::Impl::HomeFetchNext(const std::string &p_ProfileId,
                                  const std::string &p_ChatId, int p_MsgCount) {
  if (m_HomeFetchAll) {
    if ((p_ProfileId == m_CurrentChat.first) &&
      ...
      if (p_MsgCount > 0) {
        ...
        std::stack<int> &messageOffsetStack =
            m_MessageOffsetStack[p_ProfileId][p_ChatId];
...
void UiModel::Impl::MarkRead(const std::string &p_ProfileId,
                             const std::string &p_ChatId,
                             const std::string &p_MsgId, bool p_WasUnread) {
  // Generally only mark read if message was unread, except if protocol needs to
  // know every view
  const bool markReadEveryView =
      HasProtocolFeature(p_ProfileId, FeatureMarkReadEveryView);
  if (!markReadEveryView && !p_WasUnread)
    return;

  // mark_read_on_view must be enabled (default), or user has performed view end
  // of history action; page down, end, etc
  static const bool markReadOnView = UiConfig::GetBool("mark_read_on_view");
  if (!markReadOnView && !m_HistoryInteraction)
    return;

  // Terminal must be active (default), unless mark_read_when_inactive is
  // enabled
  static const bool markReadWhenInactive =
      UiConfig::GetBool("mark_read_when_inactive");
  if (!(m_TerminalActive || markReadWhenInactive))
    return;
  ...
  if ((m_CurrentChatIndex < 0) || markReadAnyChat)
    return;

  std::string senderId;
  std::unordered_map<std::string, ChatMessage> &messages =
      m_Messages[p_ProfileId][p_ChatId];
...
void UiModel::Impl::OnStatusUpdate(uint32_t p_Status) {
  ...
  if (isOnline != lastOnline) {
    static const bool desktopNotifyConnectivity =
        UiConfig::GetBool("desktop_notify_connectivity");
...
void UiModel::Impl::DownloadAttachment(
    const std::string &p_ProfileId, const std::string &p_ChatId,
...
void UiModel::Impl::OnKeyDeleteMsg() {
  ...
  if (alreadyDeleted) {
    // Message already marked deleted (by other user). Skip protocol-level
    // delete and remove locally only.
    MessageCache::DeleteOneMessage(profileId, chatId, msgId);

    std::shared_ptr<DeleteMessageNotify> deleteMessageNotify =
        std::make_shared<DeleteMessageNotify>(profileId);
  ...
  std::shared_ptr<DeleteMessageRequest> deleteMessageRequest =
      std::make_shared<DeleteMessageRequest>();
...
void UiModel::Impl::OnKeyArchiveChat() {
  ...
  std::shared_ptr<ArchiveChatRequest> archiveChatRequest =
      std::make_shared<ArchiveChatRequest>();
...
void UiModel::Impl::OnKeyPinChat() {
  ...
  std::shared_ptr<PinChatRequest> pinChatRequest =
      std::make_shared<PinChatRequest>();
...
void UiModel::Impl::OnKeyPinMsg() {
  ...
  std::shared_ptr<PinMessageRequest> pinMessageRequest =
      std::make_shared<PinMessageRequest>();
...
void UiModel::Impl::OnKeyOpenMsg() {
  ...
  static const std::string openCmd = []() {
    ...
    if (messageOpenCommand.empty()) {
      messageOpenCommand =
          std::string(getenv("PAGER") ? getenv("PAGER") : "less");
    }
...
bool UiModel::Impl::GetMessageAttachmentPath(
    std::string &p_FilePath, DownloadFileAction p_DownloadFileAction) {
  ...
  if (UiModel::Impl::IsAttachmentDownloaded(fileInfo)) {
    ...
    DownloadAttachment(profileId, chatId, msgId, fileInfo.fileId,
                       p_DownloadFileAction);
    UpdateHistory();
    LOG_DEBUG("message attachment %s download started",
              fileInfo.fileId.c_str());
  }

  return false;
}

void UiModel::Impl::OnKeyOpenAttachment(
    std::string p_FilePath /*= std::string()*/) {
  AnyUserKeyInput();
  if (p_FilePath.empty()) {
    // user-triggered call
    if (!GetMessageAttachmentPath(p_FilePath, DownloadFileActionOpen))
      return;
  } else {
...
void UiModel::Impl::OpenAttachment(const std::string &p_Path) {
  static const std::string cmdTemplate = []() {
    std::string attachmentOpenCommand =
        UiConfig::GetStr("attachment_open_command");
...
// Used when taking over terminal is disallowed or when output needs to be
// captured
bool UiModel::Impl::RunCommand(const std::string &p_Cmd,
                               std::string *p_StdOut /*= nullptr*/) {
...
void UiModel::Impl::OnKeyOpenLink() {
  ...
  std::vector<std::string> msgUrls =
      StrUtil::ExtractUrlsFromStr(mit->second.text);
  ...
  if (!linkChatId.empty()) {
    ...
    std::shared_ptr<CreateChatRequest> createChatRequest =
        std::make_shared<CreateChatRequest>();
...
std::string
UiModel::Impl::OnKeySaveAttachment(std::string p_FilePath /*= std::string()*/) {
  AnyUserKeyInput();
  if (p_FilePath.empty()) {
    // user-triggered call
    if (!GetMessageAttachmentPath(p_FilePath, DownloadFileActionSave))
      return "";
  } else {
    // protocol-triggered call
  ...
  while (FileUtil::Exists(downloadsDir + "/" + dstFileName)) {
    dstFileName = FileUtil::RemoveFileExt(srcFileName) + "_" +
                  std::to_string(i++) + FileUtil::GetFileExt(srcFileName);
  }
  ...
  if (!p_FilePaths.empty()) {
    ...
    for (auto it = p_FilePaths.begin(); it != p_FilePaths.end(); ++it) {
      ...
      std::shared_ptr<SendMessageRequest> sendMessageRequest =
          std::make_shared<SendMessageRequest>();
      sendMessageRequest->chatId = chatId;
      sendMessageRequest->chatMessage.fileInfo =
          ProtocolUtil::FileInfoToHex(fileInfo);

      static const bool transferSendCaption =
          UiConfig::GetBool("transfer_send_caption");
...
void UiModel::Impl::RequestGroupMembers(const std::string &p_ProfileId,
                                        const std::string &p_ChatId) {
...
UiModel::Impl::GetGroupMembers(const std::string &p_ProfileId,
                               const std::string &p_ChatId) {
  auto it = m_GroupMembers.find(p_ProfileId);
  ...
  std::vector<ContactInfo> contactInfos =
      MessageCache::FetchGroupMembersSync(p_ProfileId, p_ChatId);
  if (!contactInfos.empty()) {
    std::vector<std::string> memberIds;
    for (const auto &ci : contactInfos) {
      memberIds.push_back(ci.id);
      if (!ci.name.empty() && m_ContactInfos[p_ProfileId].find(ci.id) ==
                                  m_ContactInfos[p_ProfileId].end()) {
...
bool UiModel::Impl::GetChatInfoIsGroup(const std::string &p_ProfileId,
                                       const std::string &p_ChatId) {
...
UiModel::Impl::ParseMentions(const std::string &p_ProfileId,
                             const std::string &p_ChatId,
...
void UiModel::Impl::OpenCreateChat(
    const std::pair<std::string, std::string> &p_Chat) {
  ...
  std::unordered_map<std::string, ChatInfo> &profileChatInfos =
      m_ChatInfos[profileId];
...
void UiModel::Impl::FetchCachedMessage(const std::string &p_ProfileId,
                                       const std::string &p_ChatId,
                                       const std::string &p_MsgId) {
  static std::map<std::string, std::map<std::string, std::set<std::string>>>
      fetchedCache;
  std::set<std::string> &msgIdFetchedCache =
      fetchedCache[p_ProfileId][p_ChatId];
...
void UiModel::Impl::MessageHandler(
  ...
  case ConnectNotifyType: {
    std::shared_ptr<ConnectNotify> connectNotify =
        std::static_pointer_cast<ConnectNotify>(p_ServiceMessage);
    ...
    if (!HasProtocolFeature(profileId, FeatureAutoGetChatsOnLogin)) {
      std::shared_ptr<GetChatsRequest> getChatsRequest =
          std::make_shared<GetChatsRequest>();
      LOG_TRACE("get chats");
      SendProtocolRequest(profileId, getChatsRequest);
    }

    if (!HasProtocolFeature(profileId, FeatureAutoGetContactsOnLogin)) {
      std::shared_ptr<GetContactsRequest> getContactsRequest =
          std::make_shared<GetContactsRequest>();
  ...
  case NewContactsNotifyType: {
    ...
    const std::vector<ContactInfo> &contactInfos =
  ...
  case NewChatsNotifyType: {
    ...
    if (newChatsNotify->success) {
      ...
      for (auto &chatInfo : newChatsNotify->chatInfos) {
        ...
        if (IsChatForceHidden(chatInfo.id))
          continue;
  ...
  case NewMessagesNotifyType: {
    ...
    if (newMessagesNotify->success) {
      ...
      if (IsChatForceHidden(chatId))
        return;
      if (m_ChatInfos[profileId].count(chatId) &&
          m_ChatInfos[profileId][chatId].isArchived)
        return;

      std::unordered_map<std::string, ChatMessage> &messages =
          m_Messages[profileId][chatId];
      std::vector<std::string> &messageVec = m_MessageVec[profileId][chatId];
      const std::vector<ChatMessage> &chatMessages =
          newMessagesNotify->chatMessages;
      const std::string &fromMsgId = newMessagesNotify->fromMsgId;

      if (!newMessagesNotify->cached) {
        LOG_TRACE("new messages %s count %d from %s", chatId.c_str(),
                  chatMessages.size(), fromMsgId.c_str());
      } else {
        LOG_TRACE("new cached messages %s count %d from %s", chatId.c_str(),
                  chatMessages.size(), fromMsgId.c_str());
      }
      ...
      for (auto &chatMessage : chatMessages) {
        ...
        if (newMessagesNotify->sequence) {
          ...
          if ((messageTime < oldestMessageTime) || (oldestMessageTime == 0)) {
            ...
            LOG_TRACE("oldest %s at %lld", oldestMessageId.c_str(),
                      oldestMessageTime);
          }
        }
      }
      ...
      if (hasNewMessage) {
        ...
        int &messageOffset = m_MessageOffset[profileId][chatId];
        if ((profileId == m_CurrentChat.first) &&
            (chatId == m_CurrentChat.second)) {
          if (GetSelectMessageActive() &&
              (messageOffset < (int)messageVec.size())) {
        ...
        std::sort(messageVec.begin(), messageVec.end(),
                  [&](const std::string &lhs, const std::string &rhs) -> bool {
                    return messages.at(lhs).timeSent >
                           messages.at(rhs).timeSent;
                  });
        // *INDENT-ON*
        ...
        if ((profileId == m_CurrentChat.first) &&
          ...
          if (!currentMessageId.empty()) {
            auto it = std::find(messageVec.begin(), messageVec.end(),
                                currentMessageId);
            if (it != messageVec.end()) {
      ...
      std::unordered_map<std::string, ChatInfo> &chatInfos =
          m_ChatInfos[profileId];
  ...
  case SendMessageNotifyType: {
    std::shared_ptr<SendMessageNotify> sendMessageNotify =
        std::static_pointer_cast<SendMessageNotify>(p_ServiceMessage);
  ...
  case MarkMessageReadNotifyType: {
    ...
    LOG_TRACE(markMessageReadNotify->success ? "mark read ok"
                                             : "mark read failed");
  } break;
  ...
  case DeleteMessageNotifyType: {
    ...
    if (deleteMessageNotify->success) {
      ...
      std::vector<std::string> &messageVec = m_MessageVec[profileId][chatId];
      messageVec.erase(std::remove(messageVec.begin(), messageVec.end(), msgId),
                       messageVec.end());
  ...
  case SendTypingNotifyType: {
    ...
    LOG_TRACE(sendTypingNotify->success ? "send typing ok"
                                        : "send typing failed");
  } break;

  case SetStatusNotifyType: {
    std::shared_ptr<SetStatusNotify> setStatusNotify =
        std::static_pointer_cast<SetStatusNotify>(p_ServiceMessage);
  ...
  case NewMessageStatusNotifyType: {
    ...
    LOG_TRACE("new read status %s is %s", msgId.c_str(),
              (isRead ? "read" : "unread"));
    std::unordered_map<std::string, ChatMessage> &messages =
        m_Messages[profileId][chatId];
    auto mit = messages.find(msgId);
  ...
  case NewMessageIsPinnedNotifyType: {
    ...
    LOG_TRACE("new pin status %s is %s", msgId.c_str(),
  ...
  case NewMessageFileNotifyType: {
    ...
    DownloadFileAction downloadFileAction =
        newMessageFileNotify->downloadFileAction;
  ...
  case NewMessageReactionsNotifyType: {
    ...
    if (!newMessageReactionsNotify->reactions.needConsolidationWithCache) {
      ...
      LOG_TRACE("new reactions for %s in %s count %d", msgId.c_str(),
                chatId.c_str(),
  ...
  case ReceiveTypingNotifyType: {
    std::shared_ptr<ReceiveTypingNotify> receiveTypingNotify =
        std::static_pointer_cast<ReceiveTypingNotify>(p_ServiceMessage);
    ...
    LOG_TRACE("received user %s in chat %s is %s", userId.c_str(),
              chatId.c_str(), (isTyping ? "typing" : "idle"));
    if (isTyping) {
  ...
  case ReceiveStatusNotifyType: {
    std::shared_ptr<ReceiveStatusNotify> receiveStatusNotify =
        std::static_pointer_cast<ReceiveStatusNotify>(p_ServiceMessage);
  ...
  case DeleteChatNotifyType: {
    ...
    if (deleteChatNotify->success) {
      ...
      for (auto it = m_ChatVec.begin(); it != m_ChatVec.end();
           /* incremented in loop */) {
      ...
      if ((m_CurrentChat.first == profileId) &&
          (m_CurrentChat.second == chatId)) {
        m_CurrentChat = m_ChatVec.at(
            NumUtil::Bound(0, m_CurrentChatIndex, ((int)m_ChatVec.size() - 1)));
        SortChats();
        OnCurrentChatChanged();
        SetSelectMessageActive(false);
      } else {
  ...
  case UpdateMuteNotifyType: {
    std::shared_ptr<UpdateMuteNotify> updateMuteNotify =
        std::static_pointer_cast<UpdateMuteNotify>(p_ServiceMessage);
    bool isMuted = updateMuteNotify->isMuted;
    std::string chatId = updateMuteNotify->chatId;
    LOG_TRACE("mute notify %s is %s", chatId.c_str(),
  ...
  case UpdatePinNotifyType: {
    std::shared_ptr<UpdatePinNotify> updatePinNotify =
        std::static_pointer_cast<UpdatePinNotify>(p_ServiceMessage);
    ...
    LOG_TRACE("pin notify %s is %s", chatId.c_str(),
              (isPinned ? "pinned" : "unpinned"));
    if (!m_ChatInfos[profileId].count(chatId))
      break;
  ...
  case UpdateArchivedNotifyType: {
    std::shared_ptr<UpdateArchivedNotify> updateArchivedNotify =
        std::static_pointer_cast<UpdateArchivedNotify>(p_ServiceMessage);
    std::string chatId = updateArchivedNotify->chatId;
    bool isArchived = updateArchivedNotify->isArchived;
    LOG_TRACE("archived notify %s is %s", chatId.c_str(),
    ...
    if (isArchived) {
      ...
      m_ChatVec.erase(std::remove(m_ChatVec.begin(), m_ChatVec.end(),
                                  std::make_pair(profileId, chatId)),
                      m_ChatVec.end());

      if ((m_CurrentChat.first == profileId) &&
          (m_CurrentChat.second == chatId)) {
        if (!m_ChatVec.empty()) {
          m_CurrentChat = m_ChatVec.at(NumUtil::Bound(
              0, m_CurrentChatIndex, ((int)m_ChatVec.size() - 1)));
        } else {
  ...
  case AvailableReactionsNotifyType: {
    ...
    LOG_TRACE("available reactions notify %s count %d", chatId.c_str(),
              availableReactionsNotify->emojis.size());
  } break;
  ...
  case FindMessageNotifyType: {
    ...
    LOG_TRACE("find message notify %s %s", (success ? "found" : "not found"),
              msgId.c_str());
    if (!msgId.empty()) {
      const std::vector<std::string> &messageVec =
          m_MessageVec[profileId][chatId];
      int &messageOffset = m_MessageOffset[profileId][chatId];
      std::stack<int> &messageOffsetStack =
          m_MessageOffsetStack[profileId][chatId];
...
std::unordered_map<std::string, std::shared_ptr<Protocol>>
UiModel::Impl::GetProtocols() {
  return m_Protocols;
}
...
void UiModel::Impl::ProcessTimers() {
  ...
  if (elapsedMs > 200) {
    ...
    static const int autoSelectChatTimeoutSec =
        UiConfig::GetNum("auto_select_chat_timeout_sec");
    if (!IsCurrentChatSet() && (autoSelectChatTimeoutSec != 0) &&
        (m_LastSyncMessageTime != 0)) {
      int64_t elapsedSinceLastSyncMessageSec =
          (nowTimeMs - m_LastSyncMessageTime) / 1000;
...
void UiModel::Impl::SortChats() {
  ...
  std::sort(m_ChatVec.begin(), m_ChatVec.end(),
            [&](const std::pair<std::string, std::string> &lhs,
                const std::pair<std::string, std::string> &rhs) -> bool {
              ...
              if (lhsChatInfo.isPinned > rhsChatInfo.isPinned)
                return true;
              if (lhsChatInfo.isPinned < rhsChatInfo.isPinned)
                return false;
              ...
              if (!mutedPositionByTimestamp) {
                ...
                if (lhsChatInfo.isMuted < rhsChatInfo.isMuted)
                  return true;
                if (lhsChatInfo.isMuted > rhsChatInfo.isMuted)
                  return false;
              }
              ...
              if (lhsChatInfo.lastMessageTime > rhsChatInfo.lastMessageTime)
                return true;
              if (lhsChatInfo.lastMessageTime < rhsChatInfo.lastMessageTime)
                return false;

              // lower chat ids are listed first
              if (lhsChatInfo.id < rhsChatInfo.id)
                return true;
              if (lhsChatInfo.id > rhsChatInfo.id)
                return false;
...
std::string UiModel::Impl::GetLastMessageId(const std::string &p_ProfileId,
                                            const std::string &p_ChatId) {
  const std::unordered_map<std::string, ChatMessage> &messages =
      m_Messages[p_ProfileId][p_ChatId];
  const std::vector<std::string> &messageVec =
      m_MessageVec[p_ProfileId][p_ChatId];
  if (messageVec.empty())
    return std::string();
  ...
  for (const auto &messageId : messageVec) {
    ...
    if (lastChatMessage.timeSent !=
        std::numeric_limits<int64_t>::max()) // skip sponsored messages
    {
...
void UiModel::Impl::UpdateChatInfoLastMessageTime(
    const std::string &p_ProfileId, const std::string &p_ChatId) {
  const std::unordered_map<std::string, ChatMessage> &messages =
      m_Messages[p_ProfileId][p_ChatId];
  ...
  if ((m_ConnectTime.count(p_ProfileId) > 0) &&
      (lastMessageTimeSent > m_ConnectTime[p_ProfileId])) {
    if (SetCurrentChatIndexIfNotSet()) {
...
void UiModel::Impl::UpdateChatInfoIsUnread(const std::string &p_ProfileId,
                                           const std::string &p_ChatId) {
  const std::unordered_map<std::string, ChatMessage> &messages =
      m_Messages[p_ProfileId][p_ChatId];
  const std::string lastMessageId = GetLastMessageId(p_ProfileId, p_ChatId);
  ...
  if (profileChatInfos.count(p_ChatId)) {
    ...
    static const bool mutedNotifyUnread =
        UiConfig::GetBool("muted_notify_unread");
    if (mutedNotifyUnread || !profileChatInfos[p_ChatId].isMuted ||
        hasMention) {
      static const bool notifyEveryUnread =
          UiConfig::GetBool("notify_every_unread");
      if (isUnread &&
          (!profileChatInfos[p_ChatId].isUnread || notifyEveryUnread)) {
        const bool receivedAfterConnect =
            (m_ConnectTime.count(p_ProfileId) > 0) &&
            (chatMessage.timeSent > m_ConnectTime[p_ProfileId]);
        if (receivedAfterConnect) {
          static const bool terminalBellActive =
              UiConfig::GetBool("terminal_bell_active");
          static const bool terminalBellInactive =
              UiConfig::GetBool("terminal_bell_inactive");
          bool terminalBell =
              m_TerminalActive ? terminalBellActive : terminalBellInactive;
          if (terminalBell) {
            m_TriggerTerminalBell = true;
          }
          ...
          if (m_TerminalActive) {
            ...
            if (isCurrentChat) {
              static const bool desktopNotifyActiveCurrent =
                  UiConfig::GetBool("desktop_notify_active_current");
              desktopNotify = desktopNotifyActiveCurrent;
            } else {
              static const bool desktopNotifyActiveNoncurrent =
                  UiConfig::GetBool("desktop_notify_active_noncurrent");
              desktopNotify = desktopNotifyActiveNoncurrent;
            }
          } else {
            static const bool desktopNotifyInactive =
                UiConfig::GetBool("desktop_notify_inactive");
            desktopNotify = desktopNotifyInactive;
          }

          if (desktopNotify) {
            const std::string name =
                ((chatMessage.senderId == p_ChatId)
                     ? GetContactName(p_ProfileId, chatMessage.senderId)
                     : GetContactName(p_ProfileId, p_ChatId) + " - " +
                           GetContactName(p_ProfileId, chatMessage.senderId)) +
...
std::string UiModel::Impl::GetContactName(const std::string &p_ProfileId,
                                          const std::string &p_ChatId) {
...
std::string
UiModel::Impl::GetContactNameIncludingSelf(const std::string &p_ProfileId,
                                           const std::string &p_ChatId) {
  const ContactInfo &contactInfo = m_ContactInfos[p_ProfileId][p_ChatId];
  const std::string &chatName = contactInfo.name;
  if (chatName.empty()) {
...
std::string UiModel::Impl::GetContactListName(const std::string &p_ProfileId,
                                              const std::string &p_ChatId,
                                              bool p_AllowId,
                                              bool p_AllowAlias) {
  const ContactInfo &contactInfo = m_ContactInfos[p_ProfileId][p_ChatId];
...
std::string UiModel::Impl::GetContactPhone(const std::string &p_ProfileId,
                                           const std::string &p_ChatId) {
  const ContactInfo &contactInfo = m_ContactInfos[p_ProfileId][p_ChatId];
  return contactInfo.phone.empty() ? "" : "+" + contactInfo.phone;
}

bool UiModel::Impl::IsContactSelf(const std::string &p_ProfileId,
                                  const std::string &p_ContactId) {
  return m_ContactInfos[p_ProfileId][p_ContactId].isSelf;
}

int64_t UiModel::Impl::GetLastMessageTime(const std::string &p_ProfileId,
                                          const std::string &p_ChatId) {
  const ChatInfo &chatInfo = m_ChatInfos[p_ProfileId][p_ChatId];
  return chatInfo.lastMessageTime;
}

bool UiModel::Impl::GetChatIsUnread(const std::string &p_ProfileId,
                                    const std::string &p_ChatId) {
  const ChatInfo &chatInfo = m_ChatInfos[p_ProfileId][p_ChatId];
  static const bool mutedIndicateUnread =
      UiConfig::GetBool("muted_indicate_unread");
  const bool indicateUnread =
      chatInfo.isUnread && (mutedIndicateUnread || !chatInfo.isMuted);
  return indicateUnread; // @todo: handle isUnreadMention
}

std::string UiModel::Impl::GetChatStatus(const std::string &p_ProfileId,
                                         const std::string &p_ChatId) {
  std::string chatStatus;
  const std::set<std::string> &usersTyping =
      m_UsersTyping[p_ProfileId][p_ChatId];
  const ContactInfo &contactInfo = m_ContactInfos[p_ProfileId][p_ChatId];
  const ChatInfo &chatInfo = m_ChatInfos[p_ProfileId][p_ChatId];
  ...
  if (!usersTyping.empty()) {
    ...
    if (m_UserOnline[p_ProfileId][p_ChatId]) {
      ...
      int64_t timeSeen = m_UserTimeSeen[p_ProfileId].count(p_ChatId)
                             ? m_UserTimeSeen[p_ProfileId][p_ChatId]
                             : -1;
      switch (timeSeen) {
      case TimeSeenNone:
        chatStatus = "away";
      ...
      default:
        chatStatus =
            "seen " + TimeUtil::GetTimeString(timeSeen, false /* p_IsExport */);
        break;
      }
    }
  } else {
...
void UiModel::Impl::OnCurrentChatChanged() {
  LOG_TRACE("current chat %s %s", m_CurrentChat.first.c_str(),
            m_CurrentChat.second.c_str());
  SetHistoryInteraction(false);
  UpdateList();
  UpdateStatus();
  UpdateHistory();
...
void UiModel::Impl::RequestMessages(const std::string &p_ProfileId,
                                    const std::string &p_ChatId) {
  std::unordered_set<std::string> &msgFromIdsRequested =
      m_MsgFromIdsRequested[p_ProfileId][p_ChatId];
  const std::string &oldestMessageId = m_OldestMessageId[p_ProfileId][p_ChatId];
  std::string fromId = (msgFromIdsRequested.empty() || oldestMessageId.empty())
                           ? ""
                           : oldestMessageId;
  ...
  if (!oldestMessageId.empty()) {
    ...
    for (auto msgIt = messageVec.rbegin(); msgIt != messageVec.rend();
         ++msgIt) {
  ...
  const int maxHistory =
      m_HomeFetchAll ? 8 : (((GetHistoryLines() * 2) / 3) + 1);
  const int limit = std::max(0, (messageOffset + 1 + maxHistory - historySize));
  if (limit == 0) {
    LOG_TRACE("no message to request %d + %d - %d >= %d", messageOffset,
              maxHistory, historySize, limit);
    return;
  }
  ...
  const int minLimit =
      12; // hack: up to 10 tgchat messages may share same timestamp
  std::shared_ptr<GetMessagesRequest> getMessagesRequest =
      std::make_shared<GetMessagesRequest>();
  getMessagesRequest->chatId = p_ChatId;
  getMessagesRequest->fromMsgId = fromId;
  getMessagesRequest->limit = std::max(limit, minLimit);
  LOG_TRACE("request messages in %s from %s limit %d", p_ChatId.c_str(),
            fromId.c_str(), getMessagesRequest->limit);
...
void UiModel::Impl::RequestUserStatus(
    const std::pair<std::string, std::string> &p_Chat) {
  static std::set<std::pair<std::string, std::string>> requestedStatuses;
  if (requestedStatuses.count(p_Chat))
    return;
  ...
  std::shared_ptr<GetStatusRequest> getStatusRequest =
      std::make_shared<GetStatusRequest>();
...
void UiModel::Impl::ProtocolSetCurrentChat() {
  ...
  if (lastCurrentChat != m_CurrentChat) {
    ...
    std::shared_ptr<SetCurrentChatRequest> setCurrentChatRequest =
        std::make_shared<SetCurrentChatRequest>();
...
void UiModel::Impl::SetStatusOnline(const std::string &p_ProfileId,
                                    bool p_IsOnline) {
  static const bool onlineStatusShare =
      UiConfig::GetBool("online_status_share");
  if (!onlineStatusShare)
    return;

  std::shared_ptr<SetStatusRequest> setStatusRequest =
      std::make_shared<SetStatusRequest>();
  ...
  if (!p_IsOnline &&
      HasProtocolFeature(p_ProfileId, FeaturePresenceRequiresOnline)) {
    // For protocols that only deliver contact presence while we are marked
    // online, announcing offline (going away) stops further presence updates,
    // so any typing or online indicator we last received would get stuck
    // showing. Reset that stale live activity for this profile. (Protocols that
    // keep streaming presence while offline, e.g. Telegram, must not do this or
    // they would show stale data.)
    ...
    // Clear online flags so the chat status degrades from "online" to "away" /
    // "seen
    ...
    if (onlineIt != m_UserOnline.end()) {
      ...
      for (auto &userOnline : onlineIt->second) {
        if (userOnline.second) {
          ...
          // A peer flagged online was active right up to now, so refresh
          // last-seen - but only if we already hold a real timestamp for them
          // (i.e. they share last-seen). Fabricating one would disclose a
          // last-seen they chose to hide.
...
int UiModel::Impl::GetHistoryLines() { return m_View->GetHistoryLines(); }
...
int UiModel::Impl::GetScreenWidth() { return m_View->GetScreenWidth(); }

int UiModel::Impl::GetScreenHeight() { return m_View->GetScreenHeight(); }

void UiModel::Impl::SetRunning(bool p_Running) { m_Running = p_Running; }

void UiModel::Impl::ReinitView() { m_View->Init(); }
...
void UiModel::Impl::UpdateEntry() { m_View->SetEntryDirty(true); }
...
UiModel::Impl::GetChatVecLock() {
  return GetChatVec();
}

std::unordered_map<std::string, std::unordered_map<std::string, ContactInfo>>
UiModel::Impl::GetContactInfos() {
...
bool UiModel::Impl::IsCurrentChat(const std::string &p_ProfileId,
                                  const std::string &p_ChatId) {
  return ((p_ProfileId == m_CurrentChat.first) &&
          (p_ChatId == m_CurrentChat.second));
}

int &UiModel::Impl::GetCurrentChatIndex() { return m_CurrentChatIndex; }

std::unordered_map<std::string, ChatMessage> &
UiModel::Impl::GetMessages(const std::string &p_ProfileId,
                           const std::string &p_ChatId) {
  return m_Messages[p_ProfileId][p_ChatId];
}

std::vector<std::string> &
UiModel::Impl::GetMessageVec(const std::string &p_ProfileId,
                             const std::string &p_ChatId) {
  return m_MessageVec[p_ProfileId][p_ChatId];
}

int &UiModel::Impl::GetMessageOffset(const std::string &p_ProfileId,
                                     const std::string &p_ChatId) {
  return m_MessageOffset[p_ProfileId][p_ChatId];
}

bool UiModel::Impl::GetSelectMessageActive() { return m_SelectMessageActive; }
...
bool UiModel::Impl::GetListDialogActive() { return m_ListDialogActive; }
...
bool UiModel::Impl::GetFileListDialogActive() { return m_FileListDialogActive; }
...
bool UiModel::Impl::GetMessageDialogActive() { return m_MessageDialogActive; }
...
bool UiModel::Impl::GetEditMessageActive() { return m_EditMessageActive; }
...
bool UiModel::Impl::GetFindMessageActive() { return m_FindMessageActive; }
...
int UiModel::Impl::GetHelpOffset() { return m_HelpOffset; }

bool UiModel::Impl::GetEmojiEnabled() { return m_View->GetEmojiEnabled(); }

bool UiModel::Impl::GetEmojiEnabledLock() { return GetEmojiEnabled(); }

bool UiModel::Impl::IsCurrentChatSet() { return (m_CurrentChatIndex >= 0); }

bool UiModel::Impl::SetCurrentChatIndexIfNotSet() {
  if ((m_CurrentChatIndex >= 0) || (m_ChatVec.empty()))
    return false;
...
void UiModel::Impl::SetTerminalActive(bool p_TerminalActive) {
  if (p_TerminalActive != m_TerminalActive) {
    ...
    static const bool onlineStatusDynamic =
        UiConfig::GetBool("online_status_dynamic");
    ...
    if (m_TerminalActive) {
      UpdateHistory(); // refresh history as we may need to mark messages as
                       // read
    }
  }
}

void UiModel::Impl::DesktopNotify(const std::string &p_Name,
                                  const std::string &p_Text) {
  static const bool desktopNotifyEnabled =
      UiConfig::GetBool("desktop_notify_enabled");
  if (!desktopNotifyEnabled)
    return;

  static const std::string cmdTemplate = [this]() {
    std::string desktopNotifyCommand =
        UiConfig::GetStr("desktop_notify_command");
      ...
      if (RunCommand("which " + command + " | head -1", &output)) {
        if (!output.empty()) {
          if (output.find("/osascript") != std::string::npos) {
            desktopNotifyCommand = "osascript -e 'display notification \"%1: "
                                   "%2\" with title \"nchat\"'";
...
bool UiModel::Impl::IsAttachmentDownloaded(const FileInfo &p_FileInfo) {
  if (p_FileInfo.fileStatus == FileStatusNone) {
    ...
    if (filePath.empty()) {
      ...
      LOG_WARNING("message attachment %s is not an absolute path",
                  filePath.c_str());
      return false;
...
bool UiModel::Impl::IsAttachmentDownloadable(const FileInfo &p_FileInfo) {
  ...
  if (p_FileInfo.fileStatus == FileStatusNone) {
    ...
    std::string filePath = p_FileInfo.filePath;
    if (filePath.empty()) {
      LOG_WARNING("message attachment %s empty path %d", filePath.c_str(),
                  hasFileId);
      return hasFileId;
    } else if (filePath.at(0) != '/') {
      LOG_WARNING("message attachment %s is not an absolute path %d",
                  filePath.c_str(), hasFileId);
      return hasFileId;
    } else if (!FileUtil::Exists(filePath)) {
      LOG_WARNING("message attachment %s does not exist %d", filePath.c_str(),
                  hasFileId);
      return hasFileId;
    } else {
...
void UiModel::Impl::Clear(bool p_AllowUndo) {
  ...
  static const bool undoClearInputEnabled =
      (UiConfig::GetNum("undo_clear_input") == 1);
...
bool UiModel::Impl::PreEditMsg(std::string &p_ProfileId, std::string &p_ChatId,
                               std::string &p_MsgId,
  ...
  if (HasProtocolFeature(p_ProfileId, FeatureEditMessagesWithinTwoDays) &&
    ...
    return false;
  } else if (HasProtocolFeature(p_ProfileId,
...
void UiModel::Impl::StartEditMsg(const std::string &p_ProfileId,
                                 const std::string &p_ChatId,
                                 const std::string &p_MsgId) {
  AnyUserKeyInput();
...
void UiModel::Impl::SaveEditMessage() {
  ...
  if (!chatMessage.fileInfo.empty()) {
    ...
    editMessageRequest->chatMessage.fileInfo =
        ProtocolUtil::FileInfoToHex(fileInfo);
  }

  // update text content
  editMessageRequest->chatMessage.text = EntryStrToSendStr(entryStr);
  editMessageRequest->chatMessage.mentions =
      ParseMentions(profileId, chatId, editMessageRequest->chatMessage.text);
...
void UiModel::Impl::OnKeyExtEdit() {
  ...
  static const std::string editorCmd = []() {
    ...
    if (messageEditCommand.empty()) {
      messageEditCommand =
          std::string(getenv("EDITOR") ? getenv("EDITOR") : "nano");
    }
...
const std::pair<std::string, std::string> &UiModel::Impl::GetNextChat() {
  ...
  const std::pair<std::string, std::string> &nextChat =
      m_ChatVec.at(nextChatIndex);
  return nextChat;
}
...
std::string UiModel::Impl::PreExtCall() {
  ...
  const std::string phone =
      GetContactPhone(m_CurrentChat.first, m_CurrentChat.second);
  return phone;
}
...
void UiModel::Impl::SendProtocolRequest(
    const std::string &p_ProfileId, std::shared_ptr<RequestMessage> p_Request) {
...
bool UiModel::Impl::HasProtocolFeature(const std::string &p_ProfileId,
                                       ProtocolFeature p_ProtocolFeature) {
  if (!m_Protocols.count(p_ProfileId)) {
    LOG_WARNING("no profile \"%s\"", p_ProfileId.c_str());
    return false;
  }
...
std::string
UiModel::Impl::GetProfileDisplayName(const std::string &p_ProfileId) {
...
void UiModel::Impl::Quit() { m_Running = false; }
...
void UiModel::Impl::EntryConvertEmojiEnabled() {
  ...
  if (!entryStr.empty()) {
    if (!emojiEnabled) {
      ...
      entryStr = StrUtil::ToWString(
          StrUtil::Emojize(StrUtil::ToString(entryStr), true));
    }
...
void UiModel::Impl::SetProtocolUiControl(const std::string &p_ProfileId,
                                         bool &p_IsTakeControl) {
  if (p_IsTakeControl) {
    if (m_ProtocolUiControl.empty()) {
      m_ProtocolUiControl = p_ProfileId;
      LOG_TRACE("set protocol ui control ok \"%s\"",
                m_ProtocolUiControl.c_str());
    } else if (m_ProtocolUiControl == p_ProfileId) {
      LOG_TRACE("set protocol ui control ok duplicate \"%s\"",
                m_ProtocolUiControl.c_str());
    } else {
      p_IsTakeControl =
          false; // indicate failure to caller, as no point waiting under lock
      LOG_TRACE("set protocol ui control failed, owned by \"%s\"",
                m_ProtocolUiControl.c_str());
    }
  } else {
...
bool UiModel::Impl::PreReact(std::string &p_ProfileId, std::string &p_ChatId,
                             std::string &p_SenderId, std::string &p_MsgId,
                             std::string &p_SelfEmoji,
                             bool &p_HasLimitedReactions) {
  p_ProfileId = m_CurrentChat.first;
  p_ChatId = m_CurrentChat.second;
  const std::vector<std::string> &messageVec =
      m_MessageVec[p_ProfileId][p_ChatId];
  ...
  p_HasLimitedReactions =
      HasProtocolFeature(p_ProfileId, FeatureLimitedReactions);
...
void UiModel::Impl::SetReact(const std::string &p_ProfileId,
                             const std::string &p_ChatId,
                             const std::string &p_SenderId,
                             const std::string &p_MsgId,
                             const std::string &p_SelfEmoji,
                             const std::string &p_Emoji) {
...
void UiModel::Impl::GetAvailableEmojis(std::set<std::string> &p_AvailableEmojis,
                                       bool &p_Pending) {
  ...
  LOG_DEBUG("get available reactions %d pending %d", p_AvailableEmojis.size(),
            p_Pending);
}
...
void UiModel::Impl::OnKeyJumpQuoted() {
  ...
  std::shared_ptr<FindMessageRequest> findMessageRequest =
      std::make_shared<FindMessageRequest>();
...
void UiModel::Impl::ForwardMessage(
    const std::pair<std::string, std::string> &p_Chat) {
  PerformForwardMessage(p_Chat);
  ReinitView();
}

void UiModel::Impl::PerformForwardMessage(
    const std::pair<std::string, std::string> &p_Chat) {
  ChatMessage message;
  ...
  if (found) {
    // fetch recipient messages before send, to avoid fetching outgoing message
    // with temporary id
    OnCurrentChatChanged();
  } else {
    LOG_WARNING("recipient %s %s not found", p_Chat.first.c_str(),
                p_Chat.second.c_str());
    return;
  }
...
bool UiModel::Impl::IsChatForceHidden(const std::string &p_ChatId) {
  static const bool statusBroadcastHidden =
      (UiConfig::GetNum("status_broadcast") == 0);
  return statusBroadcastHidden && (p_ChatId == "status@broadcast");
}

bool UiModel::Impl::IsChatForceMuted(const std::string &p_ChatId) {
  static const bool statusBroadcastMuted =
      (UiConfig::GetNum("status_broadcast") == 1);
  return statusBroadcastMuted && (p_ChatId == "status@broadcast");
}

void UiModel::Impl::GotoChat(
    const std::pair<std::string, std::string> &p_Chat) {
  // switch current chat
  bool found = false;
  for (size_t i = 0; i < m_ChatVec.size(); ++i) {
  ...
  if (found) {
    ...
    LOG_WARNING("chat %s %s not found", p_Chat.first.c_str(),
                p_Chat.second.c_str());
  }
...
void UiModel::Impl::Draw() { m_View->Draw(); }
...
bool UiModel::Impl::AutoCompose() {
  ...
  const int messageOffset =
      GetSelectMessageActive() ? m_MessageOffset[profileId][chatId] : 0;
  ...
  static const int autoComposeHistoryCount =
      UiConfig::GetNum("auto_compose_history_count");
  for (int i = 0; (i < autoComposeHistoryCount) && (it != messageVec.end());
       ++i, ++it) {
  ...
  std::string selfName =
      GetContactNameIncludingSelf(profileId, GetSelfId(profileId));
  ...
  static const std::string autoComposeCommand =
      UiConfig::GetStr("auto_compose_command");
  std::string cmd = autoComposeCommand;
  if (cmd.empty()) {
    // use bundled compose script, extracted on each use as the temp dir is
    // transient
    const std::string scriptPath = FileUtil::GetTempDir() + "/compose";
    FileUtil::WriteFile(scriptPath, std::string((const char *)ComposeScript,
                                                sizeof(ComposeScript)));
...
void UiModel::KeyHandler(wint_t p_Key) {
  static wint_t keyVimNormal = UiKeyConfig::GetKey("vim_normal");
  ...
  static wint_t keyDecreaseListWidth =
      UiKeyConfig::GetKey("decrease_list_width");
  static wint_t keyIncreaseListWidth =
      UiKeyConfig::GetKey("increase_list_width");

  static wint_t keyExtEdit = UiKeyConfig::GetKey("ext_edit");
  static wint_t keyExtCall = UiKeyConfig::GetKey("ext_call");

  static wint_t keyOtherCommandsHelp =
      UiKeyConfig::GetKey("other_commands_help");
  ...
  {
    std::unique_lock<owned_mutex> lock(m_ModelMutex);
    if (GetImpl().VimActiveNonInsert()) {
      bool fed = GetImpl().VimFeed(p_Key);
      if (fed) return;
    }
    else if (GetImpl().VimInsertAwaitingNormal() && p_Key == keyVimNormal) {
      GetImpl().VimEnterNormal(-1);
      return;
    }
  }
...
void UiModel::GetAvailableEmojis(std::set<std::string> &p_AvailableEmojis,
                                 bool &p_Pending) {
  std::unique_lock<owned_mutex> lock(m_ModelMutex);
...
std::string UiModel::GetContactListName(const std::string &p_ProfileId,
                                        const std::string &p_ChatId,
...
bool UiModel::IsContactSelf(const std::string &p_ProfileId,
                            const std::string &p_ContactId) {
  std::unique_lock<owned_mutex> lock(m_ModelMutex);
...
void UiModel::RequestGroupMembers(const std::string &p_ProfileId,
                                  const std::string &p_ChatId) {
  std::unique_lock<owned_mutex> lock(m_ModelMutex);
...
std::vector<std::string>
UiModel::GetGroupMembers(const std::string &p_ProfileId,
                         const std::string &p_ChatId) {
  std::unique_lock<owned_mutex> lock(m_ModelMutex);
...
bool UiModel::GetChatIsUnreadLocked(const std::string &p_ProfileId,
                                    const std::string &p_ChatId) {
  nc_assert(m_ModelMutex.owns_lock());
  return GetImpl().GetChatIsUnread(p_ProfileId, p_ChatId);
}

std::string UiModel::GetChatStatusLocked(const std::string &p_ProfileId,
                                         const std::string &p_ChatId) {
  nc_assert(m_ModelMutex.owns_lock());
...
std::string UiModel::GetContactListNameLocked(const std::string &p_ProfileId,
                                              const std::string &p_ChatId,
                                              bool p_AllowId,
                                              bool p_AllowAlias) {
  nc_assert(m_ModelMutex.owns_lock());
...
std::string UiModel::GetContactNameLocked(const std::string &p_ProfileId,
                                          const std::string &p_ChatId) {
  nc_assert(m_ModelMutex.owns_lock());
  return GetImpl().GetContactName(p_ProfileId, p_ChatId);
}

std::string UiModel::GetContactPhoneLocked(const std::string &p_ProfileId,
                                           const std::string &p_ChatId) {
  nc_assert(m_ModelMutex.owns_lock());
...
int64_t UiModel::GetLastMessageTimeLocked(const std::string &p_ProfileId,
                                          const std::string &p_ChatId) {
  nc_assert(m_ModelMutex.owns_lock());
...
int UiModel::GetMessageOffsetLocked(const std::string &p_ProfileId,
                                    const std::string &p_ChatId) {
  nc_assert(m_ModelMutex.owns_lock());
...
std::unordered_map<std::string, ChatMessage> &
UiModel::GetMessagesLocked(const std::string &p_ProfileId,
                           const std::string &p_ChatId) {
  nc_assert(m_ModelMutex.owns_lock());
...
std::vector<std::string> &
UiModel::GetMessageVecLocked(const std::string &p_ProfileId,
                             const std::string &p_ChatId) {
  nc_assert(m_ModelMutex.owns_lock());
...
std::string
UiModel::GetProfileDisplayNameLocked(const std::string &p_ProfileId) {
  nc_assert(m_ModelMutex.owns_lock());
  return GetImpl().GetProfileDisplayName(p_ProfileId);
}
...
void UiModel::DownloadAttachmentLocked(
    const std::string &p_ProfileId, const std::string &p_ChatId,
    const std::string &p_MsgId, const std::string &p_FileId,
    DownloadFileAction p_DownloadFileAction) {
  nc_assert(m_ModelMutex.owns_lock());
  GetImpl().DownloadAttachment(p_ProfileId, p_ChatId, p_MsgId, p_FileId,
                               p_DownloadFileAction);
}

void UiModel::FetchCachedMessageLocked(const std::string &p_ProfileId,
                                       const std::string &p_ChatId,
                                       const std::string &p_MsgId) {
  nc_assert(m_ModelMutex.owns_lock());
...
void UiModel::MarkReadLocked(const std::string &p_ProfileId,
                             const std::string &p_ChatId,
                             const std::string &p_MsgId, bool p_WasUnread) {
  nc_assert(m_ModelMutex.owns_lock());
...
void UiModel::OnKeyGotoChat() {
  ...
  if (dialog.Run()) {
    ...
    std::pair<std::string, std::string> newCurrentChat = std::make_pair(
        selectedChatListItem.profileId, selectedChatListItem.chatId);
...
std::vector<std::string> UiModel::SelectFile() {
  ...
  static const std::string filePickerCommand =
      UiConfig::GetStr("file_picker_command");
  if (!filePickerCommand.empty()) {
    ...
    if (rv == 0) {
      ...
      if (!filesStr.empty()) {
        ...
        std::set<std::string> filePathsSet =
            ToSet(filePaths);   // hack to handle nnn's duplicate results
    ...
    static const bool filePickerPersistDir =
        UiConfig::GetBool("file_picker_persist_dir");
...
void UiModel::OnKeySelectContact() {
  ...
  if (dialog.Run()) {
    ...
    std::pair<std::string, std::string> newChat =
        std::make_pair(profileId, userId);
    std::unique_lock<owned_mutex> lock(m_ModelMutex);
...
void UiModel::OnKeySelectMention() {
  ...
  if (dialog.Run()) {
    ...
    {
      std::unique_lock<owned_mutex> lock(m_ModelMutex);
      contactName = GetImpl().GetContactListName(profileId, item.memberId,
                                                 false /*p_AllowId*/,
                                                 false /*p_AllowAlias*/);
    }
...
void UiModel::OnKeyReact() {
  ...
  {
    std::unique_lock<owned_mutex> lock(m_ModelMutex);
    if (!GetImpl().GetSelectMessageActive() || GetImpl().GetEditMessageActive())
      return;

    bool messageExists = GetImpl().PreReact(profileId, chatId, senderId, msgId,
                                            selfEmoji, hasLimitedReactions);
    if (!messageExists)
      return;
  }
  ...
  UiEmojiListDialog dialog(params, selfEmoji, true /*p_HasNone*/,
                           hasLimitedReactions);
  if (dialog.Run()) {
    std::string emoji =
        StrUtil::ToString(dialog.GetSelectedEmoji(true /*p_EmojiEnabled*/));
    std::unique_lock<owned_mutex> lock(m_ModelMutex);
...
void UiModel::OnKeyFindNext() {
  ...
  {
    std::unique_lock<owned_mutex> lock(m_ModelMutex);
    if (GetImpl().GetEditMessageActive() || GetImpl().GetFindMessageActive())
      return;
...
void UiModel::OnKeyForwardMsg() {
  ...
  if (dialog.Run()) {
    UiChatListItem selectedChatListItem = dialog.GetSelectedChatItem();
    std::pair<std::string, std::string> selectedChat = std::make_pair(
        selectedChatListItem.profileId, selectedChatListItem.chatId);
...
bool UiModel::MessageDialog(const std::string &p_Title,
                            const std::string &p_Text, float p_WReq,
                            float p_HReq) {
...
void UiModel::OnKeyPin() {
  ...
  if (pinMessage) {
    ...
    if (!hasFeature) {
      MessageDialog("Warning", "Protocol does not support message pinning.",
                    0.7, 5);
      return;
    }
    ...
    if (!hasFeature) {
      MessageDialog("Warning", "Protocol does not support chat pinning.", 0.7,
                    5);
      return;
    }
...
void UiModel::OnKeyQuit() {
  if (Status::Get(Status::FlagSyncing) & Status::FlagSyncing) {
    if (!MessageDialog("Confirmation", "Syncing in progress, confirm exit?",
                       0.75, 5)) {
      return;
    }
  }
...
void UiModel::OnKeyPaste() {
  if (Clipboard::HasImage()) {
    // Open modal dialog without model mutex held
    static const bool confirmSendPastedImage =
        UiConfig::GetBool("confirm_send_pasted_image");
    ...
    const std::string tempPath =
        FileUtil::GetTempDir() + "/clipboard-" + std::to_string(index) + ".png";
    ...
    // Delete temp clipboard images after some time, allowing for it to be sent
    // async
